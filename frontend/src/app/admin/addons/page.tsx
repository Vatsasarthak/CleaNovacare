...
import { addonsList } from '@/lib/laundryData';

export default function AddonsPage() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredAddons = addonsList.filter(s => 
  ...
  return (
    <div className="relative w-full h-full flex flex-col p-6">
      <header className="mb-6 flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-black mb-1 text-secondary">Addons Management</h1>
          <p className="text-gray-400 font-semibold text-sm">Manage extra charges and addon services.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead className="bg-gray-50 sticky top-0 z-10">
              <tr className="text-[10px] font-bold uppercase tracking-widest text-gray-400 border-b border-gray-100">
                <th className="py-4 pl-6">Addon Name</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredAddons.map((addon, index) => (
                ...
                >
                  <td className="py-4 pl-6">{addon.name}</td>
                  <td className="py-4 px-4 text-primary font-black">₹{addon.price}</td>
